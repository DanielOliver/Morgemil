...
type EntitySystem(intialEntities: seq<ComponentAggregator>) =
  let mutable _entities = [ for item in intialEntities -> (item.EntityId, item) ] |> Map.ofSeq
  ...
  member this.Add (item: ComponentAggregator) =
    let actual = if EntityId.IsDefault(item.EntityId) then {item with EntityId = _identities.Generate() } else item
