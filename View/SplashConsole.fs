...
type SplashConsole() as this =
  ...
  let TextImage (lines: seq<string>) = (lines, Vector2i(lines |> Seq.map(fun t -> t.Length) |> Seq.max, lines |> Seq.length))
  ...
  let DrawCenter(text) =
    ...
    let xOffset = if drawRectangle.Left < 0 then 0 - drawRectangle.Left else 0
