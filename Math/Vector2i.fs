...
type Vector2i = 
  struct
    ...
    //########## Operator overloads #####################################
    //Addition
    static member (+) (vec1 : Vector2i, vec2 : Vector2i) = Vector2i(vec1.X + vec2.X, vec1.Y + vec2.Y)
    ...
    member this.Length = 
      ...
      | 0.0 -> 0.0
      | x -> 
        Math.Sqrt(if x < 0.0 then Math.Abs(x)
                  else x)
    ...
    member this.Maximum(vec1 : Vector2i) = Vector2i(Math.Max(this.X, vec1.X), Math.Max(this.Y, vec1.Y))
  end
