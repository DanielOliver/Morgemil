...
let RandomPoint (rng : DefaultRNG) (area : Rectangle) = 
  Vector2i.From(Range rng area.Left area.Right, Range rng area.Top area.Bottom)

///Edge-inclusive (0,0) to vec. Assume positive
let RandomVector (rng : DefaultRNG) (vec : Vector2i) = Vector2i.From(Range rng 0 vec.X, Range rng 0 vec.Y)
