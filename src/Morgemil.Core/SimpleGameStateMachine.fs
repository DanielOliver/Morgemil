...
type SimpleGameStateMachine
    ...
    let loopWorkAgent =
        MailboxProcessor<GameStateRequest>.Start(fun inbox ->
            ...
            let rec loop (previousState: GameState) =
                async {
                    ...
                    | GameStateRequest.SetResults results ->
                        ...
                        | GameStateWaitingType.WaitingForInput entityID ->
                            currentState <- GameState.WaitingForInput(entityID, inputFunc)
            ...
            loop (GameState.WaitingForInput((EntityID -1), inputFunc)))
