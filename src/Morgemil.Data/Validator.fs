...
let inline private DefinedEnum< ^T> (value: ^T) : string option =
    if System.Enum.IsDefined(typeof< ^T>, value) then
        ...
        sprintf "Value %A is not defined for enum %s" value typeof<'T>.Name
        |> Some
...
let private ExistsInTable (id: int64) (propertyName: string) (table: IReadonlyTable<_, int64>) : string option =
    ...
    | Some _ -> None
    | None ->
        sprintf "%s with ID %i doesn't exist" propertyName id
        |> Some
...
let private ExistsInTableIfValue
    ...
    | true ->
        ...
        | Some _ -> None
        | None ->
            sprintf "%s with ID %i doesn't exist" propertyName id.Value
            |> Some
...
let private AllSatisfyCondition<'T> (values: 'T list) (description: string) (condition: 'T -> bool) : string option =
    ...
    |> function
        ...
        | x ->
            sprintf "Values [ %s ] doesn't satisfy condition: %s" x description
            |> Some
...
let private AllExistsInTable
    ...
    |> function
        | x when x.Length = 0 -> None
        | x ->
            sprintf "%s with IDs [ %s ] doesn't exist" propertyName x
            |> Some
...
let private ValidateGameDataWithTable<'T when 'T :> IRow>
    ...
    let table =
        item.Object |> Table.CreateReadonlyTable id

    let validatedItems =
        item.Object |> ValidateItems getItemErrors
    ...
    |> ValidateGameDataWithTable
        (fun acc element ->
            [ ExpectedUnique element (fun x -> x.ID) "FloorGenerationParameterID" acc
              tileTable
              |> AllExistsInTable element.Tiles "Tiles"
              tileTable
              |> ExistsInTable element.DefaultTile "DefaultTile" ])
