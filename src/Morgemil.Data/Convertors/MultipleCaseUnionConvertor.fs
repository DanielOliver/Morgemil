    ...
    override this.ReadJson(reader, t, existingValue, serializer) =
        ...
        |> Seq.filter
            (fun property ->
                property.HasValues
                && cases
                   |> Seq.exists (fun t -> t.Name.Equals(property.Name, StringComparison.OrdinalIgnoreCase)))
        |> Seq.choose
            (fun property ->
                ...
                let case =
                    cases
                    |> Seq.find (fun t -> t.Name.Equals(property.Name, StringComparison.OrdinalIgnoreCase))
                ...
                if array.HasValues then
                    ...
                    let castedItem =
                        firstItem.ToObject(firstField.PropertyType, serializer)
