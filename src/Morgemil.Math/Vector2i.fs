...
type Vector2i =
    ...
    static member create(x, y) = { Vector2i.X = x; Y = y }

    static member create(scalar) = { Vector2i.X = scalar; Y = scalar }
    ...
    static member (+)(vec1: Vector2i, vec2: Vector2i) =
        Vector2i.create (vec1.X + vec2.X, vec1.Y + vec2.Y)

    static member (+)(vec1: Vector2i, scalar) =
    ...
    static member (-)(scalar, vec1: Vector2i) =
        Vector2i.create (scalar - vec1.X, scalar - vec1.Y)
    //Multiplication
    static member (*)(vec1: Vector2i, vec2: Vector2i) =
        Vector2i.create (vec1.X * vec2.X, vec1.Y * vec2.Y)
    ...
    member this.Minimum(vec1: Vector2i) =
        Vector2i.create (Math.Min(this.X, vec1.X), Math.Min(this.Y, vec1.Y))

    ///Maximum (x,y) of both elements
    member this.Maximum(vec1: Vector2i) =
        Vector2i.create (Math.Max(this.X, vec1.X), Math.Max(this.Y, vec1.Y))
