...
type Rectangle =
    ...
    static member create(position: Vector2i, size: Vector2i) = { Position = position; Size = size }
    static member create(size: Vector2i) = Rectangle.create (Vector2i.Zero, size)

    static member create(x, y, width, height) =
        Rectangle.create (Vector2i.create (x, y), Vector2i.create (width, height))
    ...
    member this.Contains(pt: Vector2i) =
        ...
        diff_pt.X < this.Size.X
        && diff_pt.Y < this.Size.Y
        && diff_pt.X >= 0
        && diff_pt.Y >= 0
    ...
    member this.Expand(scalar) =
        Rectangle.create (this.Position - scalar, this.Size + (scalar * 2))

    ///Expands
    member this.Expand(vec: Vector2i) =
        Rectangle.create (this.Position - vec, this.Size + (vec * 2))
    ...
    static member (+)(rec1: Rectangle, rec2: Rectangle) =
        ...
        Rectangle.create (minPos, maxPos - minPos + 1)
