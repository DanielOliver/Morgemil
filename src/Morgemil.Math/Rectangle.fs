...
type Rectangle = 
  ...
  static member create(position : Vector2i, size : Vector2i) = 
    { Position = position
      Size = size }    
  static member create(size : Vector2i) = Rectangle.create(Vector2i.Zero, size)
  static member create(x, y, width, height) = Rectangle.create(Vector2i.create(x, y), Vector2i.create(width, height))
  ...
  member this.Expand(scalar) = Rectangle.create(this.Position - scalar, this.Size + (scalar * 2))
    
  ///Expands 
  member this.Expand(vec : Vector2i) = Rectangle.create(this.Position - vec, this.Size + (vec * 2))
