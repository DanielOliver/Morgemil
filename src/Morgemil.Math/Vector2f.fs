...
type Vector2f =
  ...
  //########## Operator overloads #####################################
  //Addition
  static member (+) (vec1 : Vector2f, vec2 : Vector2f) = Vector2f.create(vec1.X + vec2.X, vec1.Y + vec2.Y)
  static member (+) (vec1 : Vector2f, scalar) = Vector2f.create(vec1.X + scalar, vec1.Y + scalar)
  static member (+) (scalar : float, vec1 : Vector2f) = vec1 + scalar
  //Subtraction
  static member (-) (vec1 : Vector2f, vec2 : Vector2f) = Vector2f.create(vec1.X - vec2.X, vec1.Y - vec2.Y)
  static member (-) (vec1 : Vector2f, scalar) = Vector2f.create(vec1.X - scalar, vec1.Y - scalar)
  static member (-) (scalar, vec1 : Vector2f) = Vector2f.create(scalar - vec1.X, scalar - vec1.Y)
  //Multiplication
  static member (*) (vec1 : Vector2f, vec2 : Vector2f) = Vector2f.create(vec1.X * vec2.X, vec1.Y * vec2.Y)
  static member (*) (vec1 : Vector2f, scalar) = Vector2f.create(vec1.X * scalar, vec1.Y * scalar)
  static member (*) (scalar : float, vec1 : Vector2f) = vec1 * scalar
  ...
  member this.Minimum(vec1 : Vector2f) = Vector2f.create(Math.Min(this.X, vec1.X), Math.Min(this.Y, vec1.Y))
    
  ///Maximum (x,y) of both elements
  member this.Maximum(vec1 : Vector2f) = Vector2f.create(Math.Max(this.X, vec1.X), Math.Max(this.Y, vec1.Y))
