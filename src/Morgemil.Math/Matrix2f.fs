...
type Matrix2f =
    struct
        ...
        static member TranslationMatrix(vec2: Vector2f) =
            Matrix2f(1.0, 0.0, vec2.X, 0.0, 1.0, vec2.Y)

        static member TranslationMatrix(x: float, y: float) =
            Matrix2f.TranslationMatrix(Vector2f.create (x, y))
        //########## Operator overloads #########################################
        //Multiplication
        static member (*)(matL: Matrix2f, matR: Matrix2f) =
            Matrix2f(
                matL.a0 * matR.a0 + matL.a1 * matR.b0,
                matL.a0 * matR.a1 + matL.a1 * matR.b1,
                matL.a0 * matR.a2 + matL.a1 * matR.b2 + matL.a2,
                matL.b0 * matR.a0 + matL.b1 * matR.b0,
                matL.b0 * matR.a1 + matL.b1 * matR.b1,
                matL.b0 * matR.a2 + matL.b1 * matR.b2 + matL.a2
            )

        static member (*)(mat: Matrix2f, vec: Vector2f) =
            Vector2f.create (mat.a0 * vec.X + mat.a1 * vec.Y + mat.a2, mat.b0 * vec.X + mat.b1 * vec.Y + mat.b2)
    end
