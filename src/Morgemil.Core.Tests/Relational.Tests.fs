...
let ``Can Add items to Index`` () =
    ...
    Assert.Throws<System.Exception>(fun () -> Table.GetRowByKey exampleTable (ExampleKey 123L) |> ignore)
    |> ignore
    ...
    Assert.Throws<System.Exception>(fun () -> Table.GetRowByKey exampleTable (ExampleKey 500L) |> ignore)
    |> ignore
    ...
    for item in exampleTable |> Table.Items |> Seq.toList do
        { item with Name = item.Name + " Name" } |> Table.AddRow exampleTable

    Assert.Equal(50_000, exampleTable |> Table.Items |> Seq.length)

    for index in [ 1L .. 10_000L ] do
        index |> ExampleKey |> Table.RemoveRowByKey exampleTable
...
let ``Check Joins on readonlyArrays`` () =
    ...
    let tableOne =
        Table.CreateReadonlyTable (fun (ExampleKey value) -> value) [ exampleItem1; exampleItem2; exampleItem3 ]
