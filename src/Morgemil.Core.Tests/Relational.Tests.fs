...
let ``Can Add items to Index``() =
    ...
    let readOnlyTable = Table.CreateReadonlyTable (fun (ExampleKey value) -> value) [ exampleItem1; exampleItem2 ]
    Assert.Equal(2, readOnlyTable.Items |> Seq.length)
    Assert.Equal(exampleItem1, 500L |> ExampleKey |> Table.GetRowByKey readOnlyTable)
    Assert.Equal(exampleItem2, 501L |> ExampleKey |> Table.GetRowByKey readOnlyTable)
...
let ``Check Joins on readonlyArrays``() =
    ...
    let tableOne = Table.CreateReadonlyTable (fun (ExampleKey value) -> value) [
        exampleItem1
        exampleItem2
        exampleItem3
    ]
    let tableTwo = Table.CreateReadonlyTable (fun (ExampleKey value) -> value) [
        row1
        row2
        row3
    ]
    let tableThree = Table.CreateReadonlyTable (fun (ExampleKey value) -> value) [
        exampleItem234
    ]

    let joinedRows = TableQuery.LeftJoin tableTwo (fun t -> t.ForeignKeyID |> ExampleKey) tableOne |> Seq.toArray
